<template>
  ...
  <ul class="pick-grid" :class="{ 'tile-wide': itemShape === 'wide' }">
    <li
      v-for="item in items"
      :key="String(item.effKey) + '|' + (item.resPath || item.icon || '')"
      class="pick-item"
 ...
 * EMPTY `effects` array, OR has no resPath. The `effects` branch must come
 * before the `!resPath` fallback: templates have no top-level resPath, so
 * without it every template would be mis-classified as "none".
 */
function isNoneItem(item: Record<string, any>): boolean {
  if (!item) return false;
  if (item.label === '无效果' || item.label === 'No Effect' || item.label === 'None') return true;
