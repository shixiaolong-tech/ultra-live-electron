...
const emits = defineEmits<{
   ...
   */
  (e: 'beautyChange', payload: { cameraId: string; properties: EffectProperty[]; delta?: EffectProperty[] }): void;
  (e: 'close'): void;
}>();
  ...
  (next) => {
    ...
    store.value = createEffectStore({
      ...
      onChange: (delta, full) => {
        const cameraId = currentCameraId.value;
        if (!cameraId) return;
        emits('beautyChange', { cameraId, properties: full, delta });
      },
    });
...
function reEmitBeautyForCurrentCamera() {
  ...
  emits('beautyChange', { cameraId, properties: full });
  s.markBaseline(full);
}
</script>
