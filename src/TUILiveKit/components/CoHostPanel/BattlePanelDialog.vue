    ...
    >
      <template #host-item-actions="{ user }">
        <TUIButton
          ...
          size="small"
          type="primary"
          :disabled="props.data.hasPendingConnectionInvite"
...
// keeping a local ref.
// Also check atomic invitees so that a co-host request sent from the
// ConnectionPanel tab is reflected here — without this, switching to the
// Battle tab shows "Invite battle" for a host already pending connection.
const isPkInvited = (userId: string): boolean =>
  props.data.pendingPkInviteeUserIds.includes(userId) ||
  props.data.invitees.some(u => u.userId === userId);
