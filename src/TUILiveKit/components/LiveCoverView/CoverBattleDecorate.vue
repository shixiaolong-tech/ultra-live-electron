...
watch(
  ...
  (newId, oldId) => {
    ...
    // battleId went from non-empty to empty: battle just ended.
    if (oldId) {
...
// access to the kit's event bus. Instead we watch `battleId` going from
// non-empty to empty: at that moment the score map still carries the final
// values pushed by the main window (the snapshot watcher coalesces all
// state changes within the same flush), so the result animation can be
// computed correctly without any cross-process event subscription.
