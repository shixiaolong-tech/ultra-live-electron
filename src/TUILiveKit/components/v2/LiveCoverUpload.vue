...
import { uploadImageFile, UPLOAD_ALLOWED_MIME_TYPES, UPLOAD_MAX_FILE_SIZE_MB } from '../../../api/upload';
...
function isAspectRatioValid(width: number, height: number, type: CoverType) {
  ...
  const ratio = width / height;
  return Math.abs(ratio - getExpectedRatio(type)) <= COVER_RATIO_TOLERANCE;
}
...
.cover-placeholder-title {
  color: var(--text-color-primary, #d4def7);
  font-size: 12px;
  line-height: 16px;
}

.cover-placeholder-subtitle {
  color: var(--text-color-secondary, #9ea6bc);
  font-size: 11px;
  line-height: 14px;
}
...
.cover-tip-warning {
  color: var(--text-color-warning, #f59f0b);
}
