...
(function () {
    ...
    template.uniformJson = {
        "uniformMap": {
            "segment_body" : {
                "bg_blur_value" : [10.0, 0.0, 0.0, 0.0],
...
//    // 订阅InputEvent事件
    ...
    template.onFrameUpdate = function (currentTime, entityManager, eventManager) {
        ...
        template.uniformJson.uniformMap.segment_body.tex_rect[2] = surfaceWidth;
        template.uniformJson.uniformMap.segment_body.tex_rect[3] = surfaceHeight;

        template.customGraph.uniformJson = JSON.stringify(template.uniformJson);
        console.log("onFrameUpdate " + template.customGraph.uniformJson);
    }
}());
